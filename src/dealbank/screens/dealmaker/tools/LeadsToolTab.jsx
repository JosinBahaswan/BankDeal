...
export default function LeadsToolTab({ ctx }) {
  ...
  const filteredLeads = useMemo(() => {
    ...
    return leadRows.filter((lead) => {
      if (filterType !== "All" && lead.listType !== filterType) return false;
  ...
  }, [leadRows, filterType, leadSearch]);
  ...
  return (
    <div>
      <div style={{ fontFamily: G.serif, fontSize: isMobile ? 16 : 18, marginBottom: 4 }}>Lead Lists</div>
      <div style={{ fontSize: isMobile ? 9 : 10, color: G.muted, marginBottom: isMobile ? 12 : 14 }}>Build targeted lists, manage credits, and action leads fast.</div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "repeat(2,minmax(0,1fr))" : "repeat(auto-fit,minmax(150px,1fr))", gap: 8, marginBottom: 12 }}>
        ...
        <div style={{ ...card, textAlign: "center", display: "flex", alignItems: "center", justifyContent: "center" }}>
          <button onClick={() => setShowCreditModal(true)} style={{ ...btnG, width: isMobile ? "100%" : "auto", fontSize: 9, padding: "8px 12px" }}>Buy Credits</button>
        </div>
      </div>

      <div style={{ ...card, marginBottom: 12 }}>
        <div style={{ fontFamily: G.serif, fontSize: 15, marginBottom: 8 }}>Build a List</div>
        <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "repeat(auto-fit,minmax(170px,1fr))", gap: 8, marginBottom: 8 }}>
        ...
        <button onClick={onPullList} style={{ ...btnG, width: isMobile ? "100%" : "auto", fontSize: 9, padding: "8px 12px" }}>Pull List</button>
        {pullEstimate && (
      ...
      <DataSearchBar
        ...
        placeholder="Search by name, address, phone, status, tag, or equity"
      ...
      <div style={{ display: "grid", gap: 8 }}>
        ...
        {filteredLeads.map((lead) => (
          <div key={lead.id} style={{ ...card }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: isMobile ? "flex-start" : "center", gap: 10, marginBottom: 8, flexWrap: "wrap" }}>
              <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: isMobile ? "repeat(2,minmax(0,1fr))" : "repeat(auto-fit,minmax(120px,1fr))", gap: 6, marginBottom: 8 }}>
            ...
            <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
              <button onClick={() => setDialLead(lead)} style={{ ...btnG, flex: isMobile ? 1 : "unset", fontSize: isMobile ? 9 : 8, padding: "6px 10px" }}>Call</button>
              <button onClick={() => sendSms(lead.name)} style={{ ...btnO, flex: isMobile ? 1 : "unset", fontSize: isMobile ? 9 : 8, padding: "6px 10px" }}>SMS</button>
              <button onClick={() => toggleSave(lead.id)} style={{ ...btnO, flex: isMobile ? 1 : "unset", fontSize: isMobile ? 9 : 8, padding: "6px 10px", borderColor: savedIds.includes(lead.id) ? G.green : G.border, color: savedIds.includes(lead.id) ? G.green : G.muted }}>
      ...
      {smsToast && <div style={{ position: "fixed", bottom: isMobile ? "calc(env(safe-area-inset-bottom, 0px) + 10px)" : 14, right: isMobile ? 10 : 14, left: isMobile ? 10 : "auto", background: G.surface, border: `1px solid ${G.green}44`, borderRadius: 8, padding: "8px 12px", color: G.green, fontSize: 10 }}>{smsToast}</div>}

      {dialLead && (
        <div style={{ position: "fixed", right: isMobile ? 10 : 14, bottom: isMobile ? "calc(env(safe-area-inset-bottom, 0px) + 10px)" : 14, width: isMobile ? "calc(100vw - 20px)" : 290, maxWidth: 320, background: G.card, border: `1px solid ${G.green}44`, borderRadius: 8, padding: 12, zIndex: 20 }}>
      ...
      {showCreditModal && (
        <div style={{ position: "fixed", inset: 0, background: "#000000aa", display: "flex", alignItems: "center", justifyContent: "center", padding: 12, zIndex: 30 }}>
          <div style={{ ...card, width: "100%", maxWidth: 560, maxHeight: "calc(100vh - 24px)", overflowY: "auto" }}>
