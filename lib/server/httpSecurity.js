...
function getRedisClient() {
  const url = asText(process.env.UPSTASH_REDIS_REST_URL || process.env.UPSTASH_REDIS_URL || "");
  const token = asText(process.env.UPSTASH_REDIS_REST_TOKEN || process.env.UPSTASH_REDIS_TOKEN || "");
  if (!url || !token) return null;

  try {
    return new Redis({ url, token });
  } catch (err) {
    console.warn("httpSecurity: failed to initialize Upstash Redis client", err?.message || err);
    return null;
  }
}
...
export async function enforceRateLimit(req, res, options = {}) {
  ...
  // Try Upstash Redis first (serverless-friendly). Fallback to in-memory Map if unavailable.
  ...
  if (redis) {
    try {
      ...
      const resetAtSec = Math.ceil((Date.now() + ttl * 1000) / 1000);
      ...
      console.warn("httpSecurity: Upstash rate limit check failed, falling back to memory store", err?.message || err);
      // fall through to memory fallback
    }
  }

  // Memory fallback (best-effort; not reliable in multi-instance/serverless environments)
  const now = Date.now();
