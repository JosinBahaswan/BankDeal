...
export async function renderBundlePdfBuffer(bundle) {
  // If configured to use the pure-JS generator, prefer pdf-lib (serverless-friendly)
  if (String(process.env.PDF_GENERATOR || "").toLowerCase() === "pdf-lib") {
    const { renderBundlePdfWithPdfLib } = await import("./contractsPdfPdfLib.js");
    return renderBundlePdfWithPdfLib(bundle);
  }
