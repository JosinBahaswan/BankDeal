...
export async function renderContractPdfBuffer(html) {
  ...
  if (!executablePath) {
    throw new Error("Unable to resolve Chromium executable path for server-side PDF generation");
  }

  const launchArgs = Array.isArray(chromium.args) && chromium.args.length > 0
    ? chromium.args
