...
export function buildContractHtml(bundle) {
  ...
  return [
    ...
    "  .signature-grid { grid-template-columns: 1fr !important; gap: 8px !important; }",
    "  .signature-card { page-break-inside: avoid; padding: 8px !important; }",
    "  table { width: 100% !important; font-size: 11px !important; }",
    "  th, td { display: block !important; width: 100% !important; box-sizing: border-box; }",
    "  .sig-image { max-width: 100% !important; height: auto !important; }",
...
export async function renderContractPdfBuffer(html) {
  ...
  try {
    ...
    // If Chromium can't launch in this environment (missing shared libs),
    // optionally fall back to an external HTML->PDF service if configured.
    console.error("renderContractPdfBuffer error:", err && err.message ? err.message : err);

    const fallbackUrl = asText(process.env.CONTRACTS_PDF_FALLBACK_URL);
    if (fallbackUrl) {
      try {
        const headers = { "Content-Type": "application/json" };
        if (process.env.CONTRACTS_PDF_FALLBACK_AUTH) {
          headers.Authorization = `Bearer ${process.env.CONTRACTS_PDF_FALLBACK_AUTH}`;
        }
        ...
        if (!resp.ok) {
          ...
          throw new Error(`PDF fallback returned ${resp.status}: ${text}`);
        }
        ...
        const asTextResp = await resp.text().catch(() => "");
        throw new Error("PDF fallback did not return a PDF: " + asTextResp);
      } catch (fbErr) {
        const message = `PDF generation failed (${err && err.message ? err.message : err}). Fallback also failed (${fbErr && fbErr.message ? fbErr.message : fbErr}).`;
