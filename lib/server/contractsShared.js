...
export function normalizePem(value) {
  ...
  // If already contains a PEM header, return it
  if (replaced.includes("-----BEGIN ")) return replaced;

  // If the value looks like base64 (single-line), try to decode and use it
  const compact = raw.replace(/\s+/g, "");
  if (/^[A-Za-z0-9+/=]+$/.test(compact) && compact.length > 100) {
    try {
      const decoded = (globalThis.Buffer || Buffer).from(compact, "base64").toString("utf8");
      const asDecoded = decoded.replace(/\\n/g, "\n").trim();
      if (asDecoded.includes("-----BEGIN ")) return asDecoded;
    } catch {
      // fallthrough to return replaced
    }
  }
